	...
	func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        if let metadataObject = metadataObjects.first {
            ...
            if stringValue != outputTextView.text {
                ...
                let myStrings = stringValue.components(separatedBy: .newlines)
                outputTextView.text = myStrings.joined(separator: "\n")
                for index in 0...myStrings.count - 1 {
                    print(myStrings[index])
                    print(myStrings.count)
                }
//                /myStrings.joined(separator: ", ")
            }
